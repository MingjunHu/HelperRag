...
class ModelHandler:
    def __init__(self):
        ...
        # 从配置中读取过期时间
        expiration_time = self.config.get('session_manager').get('expiration_time_minutes', 1)
    ...
    def handle_knowage(self,input_text, session_id,keowage_id):
        bot_message = knowage_agent.chat_with_history(user_input=input_text,session_id=session_id,keowage_id=keowage_id)  # 获取机器回复
    ...
    def handle_chat(self, input_text, session_id,keowage_id="1"):
        ...
        # 生成模型响应
        response = self.handle_knowage(input_text=input_text,session_id=session_id,keowage_id=keowage_id)
        # 更新历史记录
        # self.update_history(session_id, input_text, response)
        return response
    
    # 处理用户输入的单词学习消息，并与词汇代理互动获取机器人的响应
    def handle_knowage_v2(self,input_text, context,session_id,keowage_id="1"):
        bot_message = knowage_agent.chat_with_history_v2(user_input=input_text,context=context,session_id=session_id,keowage_id="1")  # 获取机器回复
        LOG.info(f"[Knowage ChatBot]: {bot_message}")  # 记录机器人回应信息
        return bot_message

    def handle_chat_v2(self, input_text, context,session_id,keowage_id="1"):
        # 获取历史记录
        # history = self.get_history(session_id)
        # 生成模型响应
        response = self.handle_knowage_v2(input_text=input_text,context=context,session_id=session_id,keowage_id="1")
        # 更新历史记录
        # self.update_history(session_id, input_text, response)
        return response
