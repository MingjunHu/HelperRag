...
from utils.mymilvus import MyMilvus 
...
MyMilvus_client=MyMilvus(model_handler.config.get("database").get("DB_URI"),model_handler.config.get("database").get("DB_TOKEN"),model_handler.config.get("database").get("DB_COLLECTION"))
...
def chat():
    ...
    try:
        # 处理用户输入并获取模型响应,keowage_id=0为SQL使用字段全，id=1为文本用，字段不全(TuanTuan用)
...
def chat_v2():
    ...
    if not input_text or not context or not session_id:
        return jsonify({"error": "input_text and context and session_id are required"}), 400

    try:
        # 处理用户输入并获取模型响应
        response = model_handler.handle_chat_v2(input_text, context,session_id,keowage_id="1")
        return jsonify({"response": response}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
@app.route('/save_from_sql', methods=['POST'])
def save_from_sql():
    LOG.debug(f"-----------------ACtion-----------------")
    try:
        data=request.json
        ...
        if not sql_text:
            sql_text="select * from app_article"
        from_id_no=data.get('from_id_no')
        to_id_no=data.get('to_id_no')
        host=data.get("host") #"127.0.0.1"
        user=data.get("user") #"root"
        password=data.get("password") #"@#c5y1o5l"
        db=data.get("db")  #"spread"
        items_no=0
        vectory_items_no=0
        try:
            LOG.debug(f"-----------------Milvus===ACtion-----------------")
            # vector_store = Milvus(
            #     embedding_function=knowageagent.embeddings,
            #     connection_args={"uri": model_handler.config.get("database").get("DB_URI"),"token":model_handler.config.get("database").get("DB_TOKEN"),"secure": True,},
            #     collection_name=model_handler.config.get("database").get("DB_COLLECTION"),
            ...
            #     uri=model_handler.config.get("database").get("DB_URI"),
            #     token=model_handler.config.get("database").get("DB_TOKEN"),
            #     # collection_name=model_handler.config.get("database").get("DB_COLLECTION"),
            # ) 

            DB_COLLECTION=model_handler.config.get("database").get("DB_COLLECTION")
            # Milvus_Client = MilvusClient(uri=model_handler.config.get("database").get("DB_URI"), 
            #                              token=model_handler.config.get("database").get("DB_TOKEN"))
            # # 获取集合信息
            # Milvus_Client.describe_collection(collection_name=DB_COLLECTION)

            LOG.debug(f"-----------------Milvus===In-----------------")
            # database=Database(model_handler.config.get("database").get("DB_URI"),model_handler.config.get("database").get("DB_TOKEN"),model_handler.config.get("database").get("DB_COLLECTION"))
            Milvus_Client=MyMilvus_client.client
            items_no,vectory_items_no,ids=comon.get_all_itmes_by_sql_and_save_to_milvus(sql_text,from_id_no,to_id_no,vector_store,Milvus_Client,DB_COLLECTION,knowageagent.embeddings,knowageagent.bge_m3_ef,host,user,password,db)
            
        except Exception as e:
            print(f"Error sql for db insert: {e}")

        return jsonify({
            "message": f"Started vectorization for {vectory_items_no} items, and {items_no} items have been vectorized",
            "vectory_items_no": vectory_items_no,
            "items_no":items_no,
            "ids":ids
        }), 200
    except Exception as e:
            ...
            if filename.lower().endswith('.txt'):
                ...
                if file_vectorization_status.get(filename) == VECTORIZATION_STATUS["NOT_VECTORIZED"]:
                    ...
                    try:
                        ...
                        # )
                        Milvus_Client=MyMilvus_client.client 
                        LOG.debug(f"-----------------Milvus_Client_1-----------------")
                        DB_COLLECTION=model_handler.config.get("database").get("DB_COLLECTION")
                        LOG.debug(f"-----------------Milvus_Client_2-----------------")
                        comon.save_to_milvus(text,chunks,Milvus_Client,DB_COLLECTION,target_file_path,knowageagent.bge_m3_ef)
                        LOG.debug(f"-----------------Milvus_Client_3-----------------")
                        comon.movefiel_for_MakItDown(target_file_path, backup_file_path)
                        LOG.debug(f"-----------------Milvus_Client_4-----------------")
                        # 这里临时模拟处理完成
