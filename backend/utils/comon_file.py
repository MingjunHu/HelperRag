...
#关键词制造机器
    ...
    response = chat.invoke(messages)
    print("关键词有："+response.content)  # Displays the AI-generated poem
    return response.content
...
def query_by_vector_and_text(keywords,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION):
    LOG.debug("++++++++++++++++++++++++++++++++")
    documents=[]
    ids=[]
...
def query_by_doc_ids(main_ids,keywords,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION):
    documents=[]
    ids=[]
    try:
        client = MilvusClient(uri=DB_URI,token=DB_TOKEN)
...
def query_4_text(query_text,ask_mainkeyword,ask_keywords_map,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION):
#根据问题获取内容切片和切片来源（方案二）
    ...
    for ask,keyword in ask_keywords_map.items():
        ...
        hit_keywords.extend(keyword)
    # 5. 在向量和标量列中同时搜索<A> or <B> or <...>结果组成主键list<main_ids>
    LOG.debug(f"[query_4_text]-----------------5-----------------")
    main_documents,main_ids=query_by_vector_and_text(ask_mainkeyword,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION)
    ...
    LOG.debug(f"[query_4_text]-----------------7-----------------")
    hit_documents,hit_ids=query_by_doc_ids(main_ids,hit_keywords,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION)

    # 8. 将<main_ids>中的<hit_ids>删除并生成<nohit_ids>
    LOG.debug(f"[query_4_text]-----------------8-----------------")
    ...
    LOG.debug(f"[query_4_text]-----------------9-----------------")
    output_ids=hit_ids+nohit_ids
    output_documents=hit_documents+nohit_documents
...
def answer_maker(first_ask,ask_mainkeyword,ask_keywords_map,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION):
#用问题找答案资料，返回问题串，回复资料串，源文件信息，所有文本切片
    ...
    #方案2
    answer_text,file_lists,total_docs,ids,ask_texts=query_4_text(first_ask,ask_mainkeyword,ask_keywords_map,embeddings,bge_m3_ef,DB_URI,DB_TOKEN,DB_COLLECTION)
    #方案1
    ...
    if(len(total_docs)>5):
        reranker_total_docs=total_docs[:5]
    else:
